...
def ui(page: ft.Page):
    ...
    def update_cpu_info_output():
        while True:
            try:
                data = asyncio.run(fetch_cpu_data("http://localhost:5123/api/hardware/cpu"))
                if "error" not in data and data:
                    ...
                    if 'hardwareName' in cpu:
                        ...
                        if cpu_brand in SENSOR_MAPPING:
                            ...
                            temperature_sensor = next((s['value'] for s in cpu['sensors'] if s['sensorType'] == 'Temperature' and any(keyword in s['sensorName'] for keyword in mapping['temperature'])), 'N/A')
                            cpu_info_output.controls[0].value = f"{translate('cpu_temp').split(':')[0]}: {temperature_sensor:.1f} °C" if temperature_sensor != 'N/A' else f"{translate('cpu_temp').split(':')[0]}: N/A"

                            voltage_sensor = next((s['value'] for s in cpu['sensors'] if s['sensorType'] == 'Voltage' and any(keyword in s['sensorName'] for keyword in mapping['voltage'])), 'N/A')
                            cpu_info_output.controls[1].value = f"{translate('cpu_voltage').split(':')[0]}: {voltage_sensor:.2f} V" if voltage_sensor != 'N/A' else f"{translate('cpu_voltage').split(':')[0]}: N/A"

                            clock_sensor = next((s['value'] / 1000 for s in cpu['sensors'] if s['sensorType'] == 'Clock' and any(keyword in s['sensorName'] for keyword in mapping['clock'])), 'N/A')
                            cpu_info_output.controls[2].value = f"{translate('cpu_clock').split(':')[0]}: {clock_sensor:.2f} GHz" if clock_sensor != 'N/A' else f"{translate('cpu_clock').split(':')[0]}: N/A"

                            load_sensor = next((s['value'] for s in cpu['sensors'] if s['sensorType'] == 'Load' and any(keyword in s['sensorName'] for keyword in mapping['load'])), 'N/A')
                            cpu_info_output.controls[3].value = f"{translate('cpu_load').split(':')[0]}: {load_sensor:.1f} %" if load_sensor != 'N/A' else f"{translate('cpu_load').split(':')[0]}: N/A"

                            page.update()
                    else:
                        print("Error: 'hardwareName' key not found in CPU data. Please check the data structure.")
